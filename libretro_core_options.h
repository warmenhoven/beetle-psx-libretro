...
struct retro_core_option_v2_definition option_defs_us[] = {
...
#ifndef EMSCRIPTEN
   {
      ...
      "CD Access Method",
      NULL,
      "'Synchronous' mimics original hardware. 'Asynchronous' can reduce stuttering on devices with slow storage. 'Pre-Cache' loads the entire disk image into memory when launching content, which may improve in-game loading times at the cost of an initial delay at startup. 'Pre-Cache' may cause issues on systems with low RAM, and will fall back to 'Synchronous' for physical media. Restart required.",
      NULL,
      "system",
      {
         { "sync",     "Synchronous" },
...
#endif
   {
      ...
      "CD Loading Speed",
      NULL,
      "Values higher than '2x (Native)' can greatly reduce in-game loading times, but may introduce timing errors. Some games may not function properly above a certain value.",
      NULL,
      "system",
      {
         { "2x(native)", "2x (Native)" },
   ...
   },
   {
      ...
      "Override BIOS",
      NULL,
      "Override the standard region specific BIOS with a region-free one if found. Restart required.",
      NULL,
      "system",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      NULL,
      "Resolutions higher than '1x (Native)' improve fidelity of 3D models at the expense of increased performance requirements. 2D elements are generally unaffected by this setting.",
      NULL,
      "video",
      {
         { "1x(native)", "1x (Native)" },
...
#if defined(HAVE_OPENGL) || defined(HAVE_OPENGLES)
   {
      ...
      NULL,
      "Higher color depth can reduce color banding effects without the use of dithering. 16 bpp emulates original hardware but may have visible banding if dithering is not enabled. 'Dithering Pattern' is recommended to be disabled when this option is set to 32 bpp.",
      NULL,
      "video",
      {
...
#endif
   {
      ...
      "Dithering Pattern",
      NULL,
      "'1x (Native)' emulates native low resolution dithering used by original hardware to smooth out color banding artifacts visible at native color depth. 'Internal Resolution' scales dithering granularity to the configured internal resolution for cleaner results. Recommended to be disabled when running at 32 bpp color depth. Note: On Vulkan, enabling this will force downsampling to native color depth, while disabling will automatically enable output at higher color depth.",
      NULL,
      "video",
      {
         { "1x(native)",          "1x (Native)" },
...
#if defined(HAVE_OPENGL) || defined(HAVE_OPENGLES) || defined(HAVE_VULKAN)
   {
      ...
      "Texture Filtering",
      NULL,
      "'Nearest' emulates original hardware. 'Bilinear' and '3-Point' are smoothing filters, which reduce pixelation via blurring. 'SABR', 'xBR', and 'JINC2' are upscaling filters that may improve texture fidelity/sharpness at the expense of increased performance requirements. Only supported by the hardware renderers.",
      NULL,
      "video",
      {
         { "nearest",  "Nearest" },
   ...
   },
   {
      ...
      NULL,
      "MSAA improves the appearance of 3D objects. Only supported by the Vulkan renderer.",
      NULL,
      "video",
      {
         { "1x",  NULL },
...
#endif
   {
      ...
      NULL,
      "Enable dynamic calibration of analog inputs when the device is set to DualShock, Analog Controller, Analog Joystick, or neGcon. Maximum registered input values are monitored in real time and used to scale analog coordinates passed to the emulator. This should be used for games such as Mega Man Legends 2 that expect larger values than what modern controllers provide. For best results, analog sticks should be rotated at full extent to tune the calibration algorithm each time content is loaded.",
      NULL,
      "input",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      NULL,
      "The button combination that will be used to toggle between DIGITAL and ANALOG mode for the emulated DualShock.",
      NULL,
      "input",
      {
   ...
   },
   {
      ...
      NULL,
      "Hold time for the analog mode combo buttons.",
      NULL,
      "input",
      {
         { "0", "0 Second Delay" },
   ...
   },
   {
      ...
      "Gun Input Mode",
      NULL,
      "Use mouse-controlled 'Light Gun' or 'Touchscreen' input when device type is set to 'Guncon/G-Con 45' or 'Justifier'.",
      NULL,
      "input",
      {
         { "lightgun",    "Light Gun" },
   ...
   // Shouldn't the gun_input_mode just be Mouse vs. Touchscreen?
   {
      ...
      "Gun Cursor",
      NULL,
      "Draw a cursor on screen while using the 'Guncon/G-Con 45' and 'Justifier' input device types.",
      NULL,
      "input",
      {
         { "off",   "disabled" },
   ...
   },
   {
      ...
      NULL,
      "Light gun crosshair color for port 1.",
      NULL,
      "input",
      {
         { "red",    "Red" },
   ...
   },
   {
      ...
      NULL,
      "Light gun crosshair color for port 2.",
      NULL,
      "input",
      {
         { "blue",   "Blue" },
   ...
   },
   {
      ...
      "Mouse Sensitivity",
      NULL,
      "Responsiveness of the 'Mouse' input device type.",
      NULL,
      "input",
      {
         { "5%",   NULL },
   ...
   },
   {
      ...
      "Memory Card Method",
      NULL,
      "Save data format used for the left memory card slot. 'Mednafen' may be used for compatibility with the stand-alone version of Mednafen. When used with Beetle PSX, Libretro (.srm) and Mednafen (.mcr) saves have internally identical formats and can be converted between one another via renaming. Restart required.",
      NULL,
      "memcards",
      {
         { "libretro", "Libretro" },
   ...
   },
   {
      ...
      "Memory Card Slot 2",
      NULL,
      "Emulate a second memory card in the right memory card slot. Note: Some games require this option to be disabled for correct operation (e.g. Codename Tenka). Note: Memory Card Slot 2 uses the Mednafen (.mcr) save format. Restart required.",
      NULL,
      "memcards",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "Shared Memory Cards",
      NULL,
      "Use shared memory cards instead of separate cards for each content. Note: if 'Memory Card Method' is set to 'Libretro', only memory card slot 2 will be affected. Restart required.",
      NULL,
      "memcards",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "Memory Card Slot 1 Index",
      NULL,
      "Memory card currently loaded in the left slot. This option will only work if Memory Card Method is set to Mednafen.",
      NULL,
      "memcards",
      {
         { "0",  "0 (Default)" },
   ...
   },
   {
      ...
      "Memory Card Slot 2 Index",
      NULL,
      "Memory card currently loaded in the right slot. This option will only work if Memory Card Slot 2 is enabled.",
      NULL,
      "memcards",
      {
         { "0",  NULL },
   ...
   },
   {
      ...
      NULL,
      "Aspect ratio to be used by the Widescreen Mode Hack.",
      NULL,
      "hacks",
      {
         { "16:9",  NULL },
...
#endif
   {
      ...
      NULL,
      "Automatic Toggling allows the core to switch between progressive and interlaced rates.",
      NULL,
      "video",
      {
   ...
   },
   {
      ...
      "Deinterlace Method",
      NULL,
      "'Weave' shows both fields on the same frame for full high resolution still image, 'Bob' sacrificies resolution clarity for motion clarity. 'Bob (Offset)' is Bob with the current field doubled into the full surface height with a one-line offset between fields, preserving the libretro output resolution while keeping Bob-style smooth motion. 'FastMAD (Motion Adaptive)' is a CPU port of PCSX2's motion-adaptive deinterlacer: weaves still pixels and vertically-interpolates moving pixels per-pixel, full output height with neither weave's comb on motion nor bob's halved resolution. 'Off' makes the software renderer match the hardware (Vulkan/OpenGL) renderer behaviour by rasterising to all VRAM lines every frame and deferring scanout to the end of the frame, eliminating comb-on-motion in interlaced games. Note: programs that DMA to VRAM during scanout (some demos using >512-line PAL framebuffer-wrap tricks), or that update moving sprites via CPU-to-VRAM image transfers tied to the displayed field (e.g. Pro Pinball), may show stale rows or sprite ghosting under 'Off'; use 'Weave', 'Bob', 'Bob (Offset)', or 'FastMAD' for those.",
      NULL,
      "video",
      {
         { "weave",      "Weave" },
   ...
   },
   {
      ...
      NULL,
      "Due to different standards, PAL games often appear slowed down compared to the American or Japanese NTSC releases. This option can be used to override PAL timings in order to attempt to run these games with the NTSC framerate.",
      NULL,
      "video",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "Core Aspect Ratio",
      NULL,
      "This setting is ignored when the Widescreen Mode Hack or Display Full VRAM options are enabled.",
      NULL,
      "video",
      {
         { "corrected", "Corrected" },
   ...
   },
   {
      ...
      "Crop Overscan",
      NULL,
      "Full native image or no borders.",
      NULL,
      "video",
      {
         { "disabled", "Disabled" },
   ...
   },
   {
      ...
      "Additional Cropping",
      NULL,
      "Reduce the width of the cropped image by the specified number of pixels.",
      NULL,
      "video",
      {
         { "disabled", "0" },
   ...
   },
   {
      ...
      NULL,
      "Offset horizontally to the right (positive) or left (negative) by the specified number of pixels. May be used to correct alignment issues. Only supported by the software renderer.",
      NULL,
      "video",
      {
         { "-12px",    NULL },
...
#if defined(HAVE_OPENGL) || defined(HAVE_OPENGLES) || defined(HAVE_VULKAN)
   {
      ...
      NULL,
      "Positive values move image to the right, negative values move image to the left. Only supported by the hardware renderers.",
      NULL,
      "video",
      {
         { "-40",      NULL },
...
#endif
   {
      ...
      NULL,
      "First displayed scan line when running NTSC content. Values greater than zero will reduce the height of output images by cropping pixels from the topmost edge. May be used to counteract letterboxing.",
      NULL,
      "video",
      {
         { "0",  "0 (Default)" },
   ...
   },
   {
      ...
      "Last Scan Line - NTSC",
      NULL,
      "Last displayed scan line when running NTSC content. Values less than 239 will reduce the height of output images by cropping pixels from the bottommost edge. May be used to counteract letterboxing.",
      NULL,
      "video",
      {
         { "210", NULL },
   ...
   },
   {
      ...
      NULL,
      "First displayed scan line when running PAL content. Values greater than zero will reduce the height of output images by cropping pixels from the topmost edge. May be used to counteract letterboxing.",
      NULL,
      "video",
      {
         { "0",  "0 (Default)" },
   ...
   },
   {
      ...
      "Last Scan Line - PAL",
      NULL,
      "Last displayed scan line when running PAL content. Values below 287 will reduce the height of output images by cropping pixels from the bottommost edge. May be used to counteract letterboxing.",
      NULL,
      "video",
      {
         { "230", NULL },
